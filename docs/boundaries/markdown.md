...
- **图片不携带来源信息** —— `![alt](https://…)` 会渲染 `<img>`，但使用 `loading="lazy"` 与 `referrerPolicy="no-referrer"`；CSP 也只允许 `http(s)` 图片源。
...
**有意的偏差（不是缺口）**：HTML 一律按纯文本显示、链接不生成可导航 `<a>`、图片只显示替代文字且不下载。这三条让 `HTML blocks` / `Raw HTML` / `Images` 的 identical 数天生低——规范把它们变成标记，我们让它们留在页面上。kept 列才是这些节该看的数字。
