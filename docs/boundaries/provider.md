...
- **Gemini**：继续用 `inlineData:{mimeType, data}`，四种格式都走这一条路——Gemini 的 `inlineData` 本来就是「任意内联媒体」，没有第二套词汇。
- **Anthropic**：Messages API 没有音频内容块，所以带音频的请求会以 `ProviderError` 明确失败并说明原因，**不会**静默省略附件（静默省略等于让模型回答一个它没听到的问题）。
...
| `apiVersion` | 可选，仅 `anthropic`。覆盖默认的 `anthropic-version: 2023-06-01` |
...
**请求头处理顺序**（三个适配器同一条规则）：先合并 `customHeaders`；如果存在 API key，则**先删除所有大小写形式的既有凭据头**，再写入自己的那一条（`Bearer <key>` / `x-api-key` / `x-goog-api-key`）；凭据库里的 key 永远是权威来源，自定义头不能覆盖它。`anthropic` 另外无条件删掉自定义头里的 `anthropic-version`，再写入 `config.apiVersion` 或默认值，因为版本头归适配器所有。
...
**取消、超时与错误。** `ChatRequest.signal` 接到内部 `AbortController` 并传给 `fetch`，用户按停止会真正中断在途请求；超时由独立定时器触发，到期即中止。用户主动取消产出 `done` 且 `finishReason` 为 `cancelled`，超时则以 `error` 事件的形式浮出——调用方需要区分这两者。所有适配器的目录、网络层、协议流与终止错误都统一经 `safeProviderMessage()`（把 `api key`、`authorization`、`bearer`、`token` 形式的赋值替换为 `[redacted]`，截断到 300 字符）。
...
**安全的凭证边界。** Agent 每次运行只接收一份 `RuntimeProviderConfig`；API key 是唯一允许的认证材料，必须来自操作系统凭据库，不得出现在 Provider 日志、活动记录或审计里。`customHeaders` 由设置页按“名称: 值”逐行编辑，落库与运行时共用同一份白名单和非敏感值检查：值必须非空、不超过 4096 字符、不含换行，也不能以 `Bearer ` 或 `Basic ` 开头。**没有凭据引用的 Provider 只有在 `baseUrl` 指向本机时才被视为可用**（`localhost`、`127.0.0.1`、`[::1]`）；其他地址缺少密钥时会在**运行前**被判为不可用，而不是等请求失败。
