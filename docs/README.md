...
- **想知道某个形状是为什么**：[架构决策记录](./adr.md#架构决策记录adr-00010021)。代码注释里的 `ADR NNNN` 指的就是那里。
...
- **新增一条决策**时，在[架构决策记录](./adr.md#架构决策记录adr-00010021)末尾加一条，编号取下一个四位零填充数字，并保持既有的三栏结构：决定、为什么、代价。**已有记录不应被改写来掩盖变化**：决定变了就新增一条，并在旧记录的状态里标明被哪一条取代。
