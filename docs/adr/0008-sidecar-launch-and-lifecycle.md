...
- **崩溃不会被伪装成「运行完成」。** 进程消失后 supervisor 清空运行状态、记录退出信息；界面通过状态轮询发现它（运行时 1.5 秒一次，未运行时 5 秒一次），并显示重新启动入口。当前不自动重启。（**该条已变更**：[ADR 0010](0010-bounded-sidecar-recovery.md) 增加了有界的自动恢复；「崩溃不被伪装成完成」这半条仍然有效。）
