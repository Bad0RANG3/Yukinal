...
/// 「顺手」把 http 配置当成 stdio 起起来（docs/boundaries/mcp.md 的「进程生命周期仍然归 Rust」：出站网络策略还不存在）。
...
mod tests {
    ...
    fn an_http_server_is_refused_by_name_and_reason() {
        ...
        assert!(
            ...
            "the error must name the reason (docs/boundaries/mcp.md: the process lifecycle stays with Rust): {message}"
        );
    }
