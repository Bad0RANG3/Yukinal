...
/// （docs/boundaries/mcp.md 的「描述文本一律视为不可信数据」），只用于展示与诊断，
/// 任何一处都不得把它当作指令。
...
pub struct McpInitialize {
    ...
    /// 服务端自报的自由文本。docs/boundaries/mcp.md 的「描述文本一律视为不可信数据」：可以展示给用户，
    /// 绝不能拼进系统提示词当作可信上下文。
...
fn parse_tool(server_id: &str, entry: &Value) -> Result<McpToolDescriptor, McpError> {
    ...
    // （docs/boundaries/mcp.md 的「命名空间与名称冲突」）。
...
pub(super) fn parse_tools_call(server_id: &str, result: &Value) -> Result<McpToolResult, McpError> {
    ...
    // 而 docs/boundaries/mcp.md 的「输入、输出与超时由本地强制」要求的 4000 字符摘要上限发生在
    // 「把它交给模型/界面/审计之前」那一步。
