...
- **安装包配好了，但没有构建过，也没有签名。** `apps/desktop/src-tauri/tauri.conf.json` 中 `bundle.active` 已是 `true`，agent 以单文件资源随包分发，Node 运行时用用户自己的（≥ 24）；但 `tauri build` 在这里一次都没有跑完过，也没有代码签名、公证与自动更新。
...
6. `node scripts/check-packaging.mjs` —— 打包契约：`bundle.resources` 的目标位置与 `tauri.conf.json`、图标、`beforeBuildCommand` 顺序、esbuild 目标与 `engines.node` 一致，以及「安装后的 agent 目录里只有一个文件」
...
pnpm package                       # 构建安装包（会先跑门禁、再 tauri build；本机从未跑通过）
