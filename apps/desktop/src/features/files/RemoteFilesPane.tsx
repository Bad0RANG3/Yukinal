...
export function RemoteFilesPane() {
  ...
  if (!serverId) return <div className="empty-state page-empty"><Icon name="folder" size={24} /><h2>选择一台服务器</h2><p>连接服务器后浏览远程文件。</p></div>;
  if (!shell) return <div className="empty-state page-empty"><Icon name="folder" size={24} /><h2>浏览器预览模式</h2><p>远程文件需要 Tauri 原生连接。</p></div>;
  ...
  return (
    <section className="remote-files-page">
      <form className="files-toolbar" onSubmit={(event) => { event.preventDefault(); navigate(draftPath); }}>
        <button type="button" className="button-secondary" onClick={() => navigate(parent)} disabled={path === "/"} title="返回上级目录" aria-label="返回上级目录"><Icon name="arrowUp" size={14} /></button>
        ...
        <button type="button" className="button-secondary" onClick={() => void files.refetch()} disabled={files.isFetching} title="刷新远程文件" aria-label="刷新远程文件"><Icon name="refresh" size={14} />刷新</button>
      </form>
      ...
      <div className="files-layout">
        <div className="file-list-panel">
          ...
          {files.isLoading ? <p className="muted-copy" role="status">正在加载…</p> : entries.length ? <ul className="remote-file-list">{entries.map((entry) => <li key={entry.path}><button type="button" className={`remote-file-row ${selectedFile === entry.path ? "remote-file-row-selected" : ""}`} aria-current={selectedFile === entry.path ? "true" : undefined} onClick={() => open(entry)}><span className="remote-file-icon"><Icon name={entry.type === "directory" ? "folder" : "file"} size={15} /></span><span className="remote-file-name">{entry.name}</span><span className="remote-file-size">{entry.type === "directory" ? "目录" : formatSize(entry.size)}</span></button></li>)}</ul> : files.isSuccess ? <p className="muted-copy">目录为空。</p> : null}
        </div>
        <div className="file-preview-panel">
          ...
          {!selectedFile ? <div className="file-preview-empty"><Icon name="file" size={28} /><p>选择文件查看内容</p><small>目录与文件预览会分别显示在这里。</small></div> : read.isLoading ? <p className="muted-copy" role="status">正在读取…</p> : read.isError ? <div role="alert"><p className="error-copy">{read.error.message}</p><button type="button" className="button-secondary" onClick={() => void read.refetch()}>重新读取</button></div> : read.data ? <pre className="file-preview-content">{read.data.content || "（空文件）"}</pre> : null}
        </div>
      </div>
    </section>
  );
