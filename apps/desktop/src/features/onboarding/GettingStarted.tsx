...
import { useProviders, PROVIDERS_QUERY_KEY } from "../../lib/providers.js";
  ...
  return (
    <section className="getting-started" aria-labelledby="getting-started-title">
      ...
      {step === 0 ? <>
        ...
        {(!provider || editingProvider) && !providers.isLoading ? <ProviderEditor
          key={provider?.id ?? "new"}
          provider={provider}
          existingProviderIds={new Set(providers.data?.map((item) => item.id))}
          onSaved={({ provider: saved }) => {
            client.setQueryData(PROVIDERS_QUERY_KEY, (current: typeof providers.data) => [...(current ?? []).filter((item) => item.id !== saved.id), saved]);
            void client.invalidateQueries({ queryKey: PROVIDERS_QUERY_KEY });
